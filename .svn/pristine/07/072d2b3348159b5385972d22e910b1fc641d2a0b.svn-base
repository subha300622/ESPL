<%-- 
    Document   : closedIssuesChart
    Created on : 14 Jun, 2010, 12:10:55 PM
    Author     : ADAL
--%>
...
<%
    ...
    Logger logger = Logger.getLogger("ClosedIssueChart");
...
//        logger.info("Selected Month"+selectMonth);
    ...
    logger.info("diffYear" + diffYear + ", " + diffMonth);
    int newMon = month;
    if (diffMonth < 3) {
        newMon = newMon - (3 - diffMonth);
        logger.info("Start Date 1 :" + newMon);
        if (newMon < 0) {
            newMon = (total +newMon);
            year = year - 1;
        }
    ...
    //Retriving Closed Issues for all projects
    logger.info("Start Date 1 :" + startDate);
    logger.info("End Date  1  :" + endDate);
    String projectClosedIssues[][] = ProjectIssues.totalClosedIssues(startDate, endDate);
    ...
    logger.info("Start Date  :" + startDatea);
    logger.info("End Date    :" + endDatea);

    //Retriving Worked Issues for all projects
    String projectClosedIssuesa[][] = ProjectIssues.totalClosedIssues(startDatea, endDatea);
    ...
    logger.info("Start Dateb  :" + startDateb);
    logger.info("End Date b   :" + endDateb);
    String projectClosedIssuesb[][] = ProjectIssues.totalClosedIssues(startDateb, endDateb);
    ...
    logger.info("maxdayc  :" + maxdayc + ", " + monthq3);
    ...
    logger.info("Start Date3  :" + startDatec);
    logger.info("End Date 3   :" + endDatec);

    //Retriving Worked Issues for all projects
    String projectClosedIssuesc[][] = ProjectIssues.totalClosedIssues(startDatec, endDatec);
    endDateinD = sdf.parse(endDatec);

    //Retriving Worked Issues for all projects
    int noOfProjects = projectClosedIssues.length;
    int noOfProjectsa = projectClosedIssuesa.length;
    int noOfProjectsb = projectClosedIssuesb.length;
    int noOfProjectsc = projectClosedIssuesc.length;
    logger.info("noOfProjects  :" + noOfProjects);
    logger.info("noOfProjectsa  :" + noOfProjectsa);
    logger.info("noOfProjectsb :" + noOfProjectsb);
    logger.info("noOfProjectsc  :" + noOfProjectsc);
...
<html>
        ...
        <script type="text/javascript">
            function monthSelection() {
                ...
                if (selectedValue >= startRange && selectedValue <= current_date) {
                    ...
                    location = 'closedIssuesChart.jsp?month=' + month + '&year=' + year;
                } else {
                    alert("You can view Eminentlabs Closed Issues chart from Aug 2008 to Current Month");
                }
            }
    ...
    <body>
        ...
        <table width="100%" bgColor="#C3D9FF">
            <tr  bgColor="#C3D9FF">
                <td  align="left" width="55%"><font size="4" COLOR="#0000FF"><b>Closed Isssue for the Month of</b></font>
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectClosedIssues[k][1]%>",
                                                "value": "<%= projectClosedIssues[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentClosedIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectClosedIssues[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectClosedIssuesa[k][1]%>",
                                                "value": "<%= projectClosedIssuesa[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentClosedIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectClosedIssuesa[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectClosedIssuesb[k][1]%>",
                                                "value": "<%= projectClosedIssuesb[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentClosedIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectClosedIssuesb[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectClosedIssuesc[k][1]%>",
                                                "value": "<%= projectClosedIssuesc[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentClosedIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectClosedIssuesc[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
