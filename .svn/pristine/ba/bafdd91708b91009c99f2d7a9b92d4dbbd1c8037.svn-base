<%-- 
    Document   : dashboardForSC
    Created on : 5 Nov, 2011, 9:51:46 PM
    Author     : Tamilvelan
--%>
...
<html>
    ...
    <body>
        ...
        <table>
            ...
            <tr>
                <td><span title="Client"><%=clientName%></span> >> <span title="Company Code"><%=compName%></span> >> <span title="Business Process"><%=nameofbp%></span> >> <span title="Main Process"><%=nameofmp%></span> >> <span title="Sub Process"><%=nameofsp%></span>  >> <b>Scenario</b></td>
            </tr>
        </table>
        <%
            int sizeOftheSC = scenario.size();
            if (sizeOftheSC > 0) {
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                            {
                                "label": "<%=nameofsc%>",
                                "value": "<%= vtCount%>",
                                "link": '<%= request.getContextPath()%>/UserBPM/dashboardForVT.jsp?scId=<%=key%>&pid=<%=pid%>&scName=<%=nameofsc%>&spName=<%=nameofsp%>&mpName=<%=nameofmp%>&bpName=<%=nameofbp%>&companyName=<%=compName%>&clientName=<%=clientName%>'
                                                    },
            <%}%>
                                                ]
                                            }
        ...
        <table cellpadding="0" cellspacing="0" width="100%" datapagesize="25">
            <tr>
                <td  align="center" width="100%"><font size="4" COLOR="#0000FF"><b>No Scenario Available</b></font></td>
            </tr>
        </table>
        <br>
        <br>
