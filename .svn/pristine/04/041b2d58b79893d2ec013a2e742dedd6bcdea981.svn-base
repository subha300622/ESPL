...
<table width="100%" align=center border="0" bgcolor="#F9F9F9">
	<tr>
		...
		Moved a Contact to Lead : </FONT><FONT SIZE="4" COLOR="#0000FF"> <%=request.getParameter("contact")%> </FONT></td>
	</tr>
</table>
<%
	}
...
<% 
	session.setAttribute("forwardpage","viewcontact.jsp");
			...
			st = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
                        if(contactName != null){
                            if(!contactName.equalsIgnoreCase("")){
                                rs = st.executeQuery("SELECT CONTACTID,FIRSTNAME, LASTNAME, ACCOUNTNAME, TITLE,PHONE,MOBILE,EMAIL,CREATEDON,DUEDATE,ASSIGNEDTO,CONTACT_OWNER,MODIFIEDON,contact_type from CONTACT where upper(firstname) like upper('%"+contactName+"%"+"') or upper(lastname) like upper('%"+contactName+"%"+"') or upper(accountname) like upper('%"+contactName+"%"+"') order by DUEDATE ASC ");
                                }else{
                                rs = st.executeQuery("SELECT contactid,FIRSTNAME,LASTNAME,ACCOUNTNAME,TITLE,PHONE,MOBILE,EMAIL,CREATEDON,DUEDATE,ASSIGNEDTO,CONTACT_OWNER,MODIFIEDON,contact_type from CONTACT where roleid=2 order by DUEDATE ASC ");
                                }
                                }else{
                                rs = st.executeQuery("SELECT contactid,FIRSTNAME,LASTNAME,ACCOUNTNAME,TITLE,PHONE,MOBILE,EMAIL,CREATEDON,DUEDATE,ASSIGNEDTO,CONTACT_OWNER,MODIFIEDON,contact_type from CONTACT where roleid=2 order by DUEDATE ASC ");
                                }
...
<TABLE width="100%" border="0">
	...
	<tr>
		<td><a href="<%=request.getContextPath() %>/admin/customer/addnewcontact.jsp">Add Contact</a>&nbsp;&nbsp;&nbsp;&nbsp;
                    <b><a href="javascript:void(0)">View Contacts</a></b>&nbsp;&nbsp;&nbsp;&nbsp; 
			<a href="<%=request.getContextPath() %>/admin/customer/waitingContacts.jsp">Approve	Contact</a>&nbsp;&nbsp;&nbsp;&nbsp; 
			<a href="<%=request.getContextPath() %>/admin/customer/deniedContacts.jsp">Denied Contact</a>&nbsp;&nbsp;&nbsp;&nbsp; 
			<a href="<%=request.getContextPath() %>/admin/customer/disabledContacts.jsp">Disabled Contact</a>
		</td>
...
%>
		<td width="20%" title="<%= StringUtil.encodeHtmlTag(title) %>"><input type="checkbox" name="approve" value="<%= email %>"><font
			face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A HREF="editContact.jsp?contactid=<%=contactid%>"><%= fullname %></A></font>
		...
		<%
                                                        ...
                                                        %>
		<td width="20%" title="<%= StringUtil.encodeHtmlTag(title) %>"><input type="checkbox" name="approve" value="<%= email %>"><font
			face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A HREF="editContact.jsp?contactid=<%=contactid%>"><%= fullname.substring(0,22)+"..."%></A></font>
...
%>
	<table align=right>
		<tr>
			<td>First</td>
			<td>Prev</td>
			<td><a href=viewcontact.jsp?manipulation=3>Next</a></td>
			<td><a href=viewcontact.jsp?manipulation=4>Last</a></td>
		</tr>
	</table>
	<%
			}
...
%>
	<table align=right>
		<tr>
			<td><a href=viewcontact.jsp?manipulation=1>First</a></td>
			<td><a href=viewcontact.jsp?manipulation=2>Prev</a></td>
			<td>Next</td>
			<td>Last</td>
		</tr>
	</table>
