<%@ page language="java" contentType="text/html"
	pageEncoding="UTF-8"%>
<%@ page import="org.apache.log4j.*,pack.eminent.encryption.*,com.eminent.util.*,java.util.LinkedHashMap,java.util.Map,java.util.HashMap,com.eminentlabs.crm.CRMUtil"%>
<%
	//Configuring log4j properties
	
	
	
	Logger logger = Logger.getLogger("My CRM waiting Contact");
...
<% 
	...
	session.setAttribute("forwardpage","/MyCRM/waitingContacts.jsp");
		...
		if(connection != null)  
		{
		   ...
		   catch(Exception e)
		   {
		   		logger.error("Exception:"+e);
                       ...
                       int apmIssues=CRMIssue.getAPMIssues(connection,userId);
                       HashMap hm=CRMIssue.getCRMIssuesCounts(connection,userId);
                       int contact=(Integer)hm.get("Contact");
                       int lead=(Integer)hm.get("Lead");
                       int opportunity=(Integer)hm.get("Opportunity");
                       int accountno=(Integer)hm.get("Account");
...
%>
		...
		<td  colspan=3  >This list shows &nbsp;<%=issuenofrom%>&nbsp;-<%=issuenoto%>&nbsp;of&nbsp;<b><%=rowcount%></b> contacts waiting to be approved.</td>
...
<br>
<form name="showleads" action="<%=request.getContextPath() %>/MyCRM/approveContacts.jsp" onSubmit="return ValidateForm(this,'approve')">
<table width=100%>
...
%>
		<td width="20%"><input type="checkbox" name="approve" value="<%= email %>"><font
			...
			HREF="viewUnApprovedContact.jsp?contactid=<%=contactid%>"><%= StringUtil.encodeHtmlTag(fullname) %></A></font></td>
		<%
                                                        }
                                                        else
                                                            {
