<%-- 
    Document   : addSeqTC
    Created on : 18 Aug, 2010, 11:13:26 AM
    Author     : Tamilvelan
--%>
	...
	function ValidateForm(dml,chkName)
	{
			...
			if ((document.theForm.elements[i].name==chkName) && (document.theForm.elements[i].checked==1)){
                ...
                if( flag == true){
                          ...
                          count = (document.theForm.elements.length-5)/4;
                          check = true;
                       for( j = 0 ; j < count ; j++) {
                           var box = (j*4) + 4;
                            ...
                            if ((document.theForm.elements[box].id == 'ptcid') && (document.theForm.elements[box].checked)) {
                                    ...
                                    if (document.theForm.elements[box+3].value=="--Select One--") {

                                         alert('Please select Assigned To for the selected Test Cases');
                                         document.theForm.elements[box+3].focus();
                                         return false;
                                    }
...
<br>
<form name="theForm" action="addSeqTC.jsp" onsubmit="return ValidateForm('theForm','ptcid')" method="post">
<table width="100%">

    <tr bgcolor="#C3D9FF" >
        <td width="10%"><b>TestCaseId</b></td>
        <td width="3%" title="Sequence Id"><b>Seq</b></td>
...
<td><input type="text" name="seqid" id="seqid" value="<%=seq++%>" size="1"></td>
        <td><%=project%></td>
        <td><%=module%></td>
