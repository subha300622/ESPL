...
<%Logger logger = Logger.getLogger("IssueNoSearch");
...
<%@ page import="org.apache.log4j.*,java.sql.*,pack.eminent.encryption.*,java.text.*,com.pack.*"%>
...
<%
	...
	rs = stmt.executeQuery("SELECT CUSTOMER,PROJECT,PLATFORM,MODULE,found_version,SEVERITY,PRIORITY,TYPE,FIX_VERSION FROM ISSUE WHERE ISSUEID='"+no+"' ");//CORRECT
	if(rs!=null)
	{
		while(rs.next())
		{
						...
						String full="";
							...
							while(rs4.next())
								...
								String des=(String)rs4.getString("description");
                                                                ...
                                                                ResultSet rs5 = stmt5.executeQuery("SELECT distinct project.category from issue,project where issue.project=project.pname and issue.project='"+pro+"'");//CORRECT
                                                                if(rs5!=null)
                                                                {
                                                                    if(rs5.next())
                                                                    {
...
//                                                                            System.out.println(rs7.getString(1)+subphase+subsubphase+subsubsubphase);
                                                                        }
                                                                    }
                                                                }
%>
