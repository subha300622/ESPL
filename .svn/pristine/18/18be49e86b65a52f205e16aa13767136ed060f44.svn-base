<%-- 
    Document   : projectIssuesChart
    Created on : 11 Jun, 2010, 8:54:29 AM
    Author     : ADAL
--%>
...
<%
    ...
    Logger logger = Logger.getLogger("ProjectIssueChart");
...
//        logger.info("Selected Month"+selectMonth);
    ...
    String projectOpenIssues[][] = ProjectIssues.totalOpenIssues(startDate, endDate);
    ...
    String projectOpenIssuesa[][] = ProjectIssues.totalOpenIssues(startDatea, endDatea);
    ...
    logger.info("Start Date  :" + startDate);
    logger.info("End Date    :" + endDate);
    String projectOpenIssuesb[][] = ProjectIssues.totalOpenIssues(startDateb, endDateb);
    ...
    //Retriving Worked Issues for all projects
    String projectOpenIssuesc[][] = ProjectIssues.totalOpenIssues(startDatec, endDatec);
    endDateinD = sdf.parse(endDatec);
    int noOfProjects = projectOpenIssues.length;
    int noOfProjectsa = projectOpenIssuesa.length;
    int noOfProjectsb = projectOpenIssuesb.length;
    int noOfProjectsc = projectOpenIssuesc.length;
...
<html>
    <meta name="keywords" content="GST in SAP, SAP GST Accelerator, GST Success in SAP, GST India in SAP, GST SAP Success, GST Suvidha Provider, GST ASP GSP, GST, CGST,SGST, IGST, UGST,  SAP NW based Product Development, SAP Consulting, SAP Implementation, SAP Support Services, SAP Outsourcing, SAP Implementation India, SAP Implementation Bangalore, SAP Implementation Chennai, SAP Implementation Belgium, SAP Implementation Paris, SAP Implemtation Australia, SAP Implementation France, SAP Implementation Mumbai, SAP Implementation Kerala, SAP Implementation Belgaum, SAP Implementation Thailand, SAP Implementation Mexico, SAP Implementation America">
        ...
        <script type="text/javascript">
                    ...
                    document.getElementById('button').value = 'Processing';
                            ...
                            location = 'projectIssuesChart.jsp?month=' + month + '&year=' + year;
                    } else {
                    alert("You can view Project Open Issues chart from Aug 2008 to Current Month");
                    }
                    }
    ...
    <body>
        ...
        <table width="100%" bgColor="#C3D9FF">
            <tr  bgColor="#C3D9FF">
                <td  align="left" width="55%"><font size="4" COLOR="#0000FF"><b>Open Isssue for the Month of</b></font>
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectOpenIssues[k][1]%>",
                                                "value": "<%= projectOpenIssues[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentOpenIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectOpenIssues[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectOpenIssuesa[k][1]%>",
                                                "value": "<%= projectOpenIssuesa[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentOpenIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectOpenIssuesa[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectOpenIssuesb[k][1]%>",
                                                "value": "<%= projectOpenIssuesb[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentOpenIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectOpenIssuesb[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                                    {
                                        "label": "<%=projectOpenIssuesc[k][1]%>",
                                                "value": "<%= projectOpenIssuesc[k][2]%>",
                                                "link":'<%= request.getContextPath()%>/admin/user/eminentOpenIssues.jsp?month=<%=month%>&year=<%=year%>&pid=<%=projectOpenIssuesc[k][0]%>'
                                                                        },
            <% j++;
            }%>
                                                                        ]
