<%-- 
    Document   : ptcmChart
    Created on : 27 Jul, 2010, 11:46:37 AM
    Author     : Tamilvelan
--%>
...
<%
    ...
    Logger logger = Logger.getLogger("PTCM Chart");
...
<html>
    ...
    <body>
        <%
            ...
            logger.info("Pid--->" + pid);
            String projectTestCases[][] = TestCases.showProjectTestCases();
            noOfTestcases = projectTestCases.length;
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                            {
                                "label": "<%=projectTestCases[k][1]%>",
                                "value": "<%= projectTestCases[k][2]%>",
                                "link": '<%= request.getContextPath()%>/admin/dashboard/TestCasesChart.jsp?project=<%=projectTestCases[k][0]%>&pid=<%=pid%>'
                                                    },
            <%}%>
                                                ]
                                            }
