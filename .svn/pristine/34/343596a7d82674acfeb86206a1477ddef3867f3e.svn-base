...
<%
			...
			//Logger logger = Logger.getLogger("validate");
	...
	<TBODY height="100%">
		<TR>
			<TD width="39%"><img border="0"
				src="../eminentech support files/eminentlogo1.gif"></TD>
...
<TABLE align="center">
	<TBODY>
		<% 
			//logger.debug("answer:"+request.getParameter("answer"));
			//logger.debug("dbanswer:"+(String)session.getAttribute("answer"));

		if(request.getParameter("answer").equalsIgnoreCase((String)session.getAttribute("answer")))
		{
			//logger.debug("==============forwarding to changepassword=============");
			%>
		...
		<jsp:forward page="getsecrect.jsp">
			<jsp:param name="answerStatus" value="false" />
