...
<%
	...
	Logger logger = Logger.getLogger("ViewOpportunity");
...
<table cellpadding="0" cellspacing="0" width="100%">
	<tr border="2" bgcolor="#E8EEF7">
		...
		<td><input type="reset" name="reset" id="submit" value="Reset"></td>
...
<% 
	...
	try
    {
		...
		if(connection != null)  
		{
			...
			rs = st.executeQuery("SELECT OPPORTUNITYID,OPPORTUNITYNAME, ASSIGNEDTO, AMOUNT, CLOSE_DATE,STAGE,PROBABILITY,MODIFIEDON from opportunity where roleid=2  and upper(stage)!=upper('Closed Lost') order by CLOSE_DATE ASC ");
			rs.last();
...
<TABLE width="100%" border="0">
    <tr>
        <td>
             <b><a href="javascript:void(0)">View Opportunities</a></b>&nbsp;&nbsp;&nbsp;&nbsp;
            <a	href="<%=request.getContextPath() %>/admin/customer/lostOpportunities.jsp">View Lost Opportunities</a>
        </td>
    </tr>
	<tr>
		...
		<%
			...
			{
%>
		<td>This list shows &nbsp;<%= "0"%>&nbsp;-<%=issuenoto%>&nbsp;of&nbsp;<%=rowcount%>&nbsp; <b>Opportunities</b> Available</td>
		<%
			}
			else
			{
%>
		<td>This list shows&nbsp;<%=issuenofrom%>&nbsp;-<%=issuenoto%>&nbsp;of&nbsp;<%=rowcount%>&nbsp; <b>Opportunities</b> Available</td>
		<%
			}
%>
	</tr>
...
%>
		<td width="25%"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A HREF="editOpportunity.jsp?opportunityid=<%=opportunityid%>"><%= StringUtil.encodeHtmlTag(opportunityname)%></A></font></td>
		...
		<td width="15%"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%=dealvalue %></font></td>
                <td width="15%" title="<%=modifiedOn%>"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%=closedate%></font></td>
	...
	<% 
			...
			if(request.getParameter("manipulation")==null && (rowcount/100==0))
			{
%>
	<table align=right>
		<tr>
