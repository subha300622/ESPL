 ...
 */
public class AutoIssueCreationController {

    static org.apache.log4j.Logger logger = null;

    static {
        logger = org.apache.log4j.Logger.getLogger("AutoIssueCreationController");
    }

    public String createIssueForAbnormalValues(String abnormals, String pid, String uid) {
        String issueidFormat = null;
        String to = "", from = "", result;
        String pmanager = null;
        IssueDAO issueDAO = new IssueDAOImpl();
        try {
            String projName = GetProjects.getProjectName(pid);
            IssueFormBean ifb = new IssueFormBean();
            ifb.setSubject("Abnormal Values in Daily Monitoring");
            ifb.setpName(projName.split(":")[0]);
            ifb.setVersion(projName.split(":")[1]);
            ifb.setType("Bug");
            ifb.setSeverity("S1- Fatal");
            ifb.setPriority("P1-Now");
            ifb.setmName("BASIS");
            ifb.setRootCause("Found Abnormal Values In Daily Monitoring");
            ifb.setCreatedBy(uid);
            ifb.setDescription(abnormals);
