...
<%@ page import="com.eminent.util.StatusSelector,java.io.*,pack.eminent.encryption.*,org.apache.log4j.*,org.apache.commons.fileupload.*,org.apache.commons.fileupload.servlet.*,org.apache.commons.fileupload.disk.DiskFileItemFactory, org.apache.commons.io.FilenameUtils,java.util.*,com.pack.StringUtil"%>
...
--> <%
		...
		<%
                    ...
                    if (fileItem.getSize() > 0& fileItem.getSize()<12582912)
                    {
                     ...
                     String issueid1=(String)session.getAttribute("issueid");
                     ...
                     boolean dir=outputFile1.mkdir();
                      ...
                      int count=IssueDetails.displayFiles(issueid1);
                      count=count+1;
                      File saveTo = new File(dirName +count+"."+issueid1+"_"+StringUtil.convertSpecialCharacters(fileName));
                      saveFile=count+"."+issueid1+"_"+StringUtil.convertSpecialCharacters(fileName);
		...
		Timestamp ts = new Timestamp(d.getYear(),d.getMonth(),d.getDate(),d.getHours(),d.getMinutes(),d.getSeconds(),d.getSeconds());
                String no1=(String) session.getAttribute("issueid");
		int owner=((Integer)session.getAttribute("uid")).intValue();
                String status=StatusSelector.getStatus(no1);
		if(rs11!=null)
		{
			while(rs11.next())
			{
