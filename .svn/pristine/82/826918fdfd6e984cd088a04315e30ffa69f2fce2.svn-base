<%-- 
    Document   : dashboardForTS
    Created on : 5 Nov, 2011, 9:56:16 PM
    Author     : Tamilvelan
--%>
...
<html>
    ...
    <body>
        <%
            ...
            HashMap tstep = ViewBPM.getTS(Integer.parseInt(tcId));
        ...
        <table>
            ...
            <tr>
                <td><span title="Client"><%=clientName%></span> >> <span title="Company Code"><%=compName%></span> >> <span title="Business Process"><%=nameofbp%></span> >> <span title="Main Process"><%=nameofmp%></span> >> <span title="Sub Process"><%=nameofsp%></span>  >> <span title="Scenario"><%=nameofsc%></span> >> <span title="Variant"><%=nameofvt%></span> >> <span title="Test Case"><%=nameoftc%> </span> >> <b>Test Step</b></td>
            </tr>
        </table>
        <%
            int sizeOftheTS = teststep.size();
            if (sizeOftheTS > 0) {
        %>
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            <%
                ...
                while (ite.hasNext()) {
                    ...
                    String nameofts = (String) teststep.get(key);
                    logger.info("Value===>>" + nameofts);
            ...
            %>
                            {
                                "label": "<%=nameofts%>",
                                "value": "<%= tsCount%>",
                                "link": '<%= request.getContextPath()%>/UserBPM/showTestScript.jsp?tsId=<%=key%>&pid=<%=pid%>&tsName=<%=nameofts%>&tcName=<%=nameoftc%>&vtName=<%=nameofvt%>&scName=<%=nameofsc%>&spName=<%=nameofsp%>&mpName=<%=nameofmp%>&bpName=<%=nameofbp%>&companyName=<%=compName%>&clientName=<%=clientName%>'
                                                    },
            <%}%>
                                                ]
                                            }
        ...
        <table cellpadding="0" cellspacing="0" width="100%" datapagesize="25">
            <tr>
                <td  align="center" width="100%"><font size="4" COLOR="#0000FF"><b>No Test Step Available</b></font></td>
            </tr>
        </table>
        <br>
        <br>
