<%-- 
    Document   : eminentClosedIssues
    Created on : Nov 23, 2009, 8:02:51 PM
...
<%
	...
	Logger logger = Logger.getLogger("eminentClosedIssues");
...
<script>
    function monthSelection(){
        ...
        if(selectedValue>=startRange &&selectedValue<=current_date ){
            ...
            alert("You can view Closed Issues from Aug 2008 to Current Month");
        }
    }
...
<%!
		...
		int i,year,month,timeSheetMonth,timeSheetYear,rowcount;
...
//        logger.info("Selected End Date"+selectedEndDate);
          ...
          String issueDetails[][]   =   IssueDetails.displayEminentIssues("Closed",start, end, projectId);
...
<table width="100%">
	<tr height="10">
            <td align="left" width="100%"><b><%=GetProjects.getProjectName(projectId).replace(":"," v")%></b> closed issue in the month of <b><%=monthSelect.get(month)%> <%=year%></b> is listed below. Total closed issues are <b> <%=rowcount%>.
		</b></td>
...
<table width="100%" height="23">
	<TR bgcolor="#C3D9FF">
		...
		<TD width="7%"><font><b>Refer</b></font></TD>
                <TD width="12%"><font><b>Rating</b></font></TD>
...
%>
		...
		<%
                                                            ...
                                                            %>
                <td width="10%" title="Created By <%=GetName.getUserName(createdBy)%>"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%=assignedTo%>
		</font></td>
                <%
