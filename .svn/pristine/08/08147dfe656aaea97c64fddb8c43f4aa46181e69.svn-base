...
function collapse(id,duration,step_duration){

	var el  = document.getElementById(id);
        var sub = document.getElementById('map');
	...
	if (el.style.height == '0px') {
        ...
        if(sub==undefined){
            ...
            if (sub.style.height == '0px') {
                    ...
                    if (!sub.morph_data) sub.morph_data = new Object();
                    sub.morph_data.naturalHeight = sub.offsetHeight;
                    sub.morph_data.style_height = sub.style.height;
                    morph_height(sub,sub.offsetHeight,0,duration,steps,1);
            }
        }
}
...
if (noscript_css) {
	...
	noscript_css.href='/eTracker/eminentech support files/loading.css';
}
