<%-- 
    Document   : accounts
    Created on : Feb 27, 2012, 4:54:24 PM
    Author     : Tamilvelan
--%>
...
<%
	...
	try
    {
		...
		if(connection != null)
		{
			...
			rs = st.executeQuery("SELECT ACCOUNTID,ACCOUNTNAME,BILLINGSTATE,PHONE,TYPE,DUEDATE,ACCOUNT_AMOUNT,ASSIGNEDTO,CREATEDON,OPPORTUNITY_REFERENCE from account where ACCOUNTID IN (SELECT DISTINCT ACCOUNTID FROM ACCOUNT_COMMENTS WHERE TO_DATE(TO_CHAR(COMMENT_DATE,'DD-MON-YY'),'DD-MON-YY') BETWEEN '"+start+"' AND '"+end+"' AND COMMENTEDBY='"+userId+"')  OR (ACCOUNT_OWNER='"+userId+"' AND TO_DATE(TO_CHAR(CREATEDON,'DD-MON-YY'),'DD-MON-YY') BETWEEN '"+start+"' AND '"+end+"' ) ORDER BY DUEDATE ASC");
			rs.last();
...
<TABLE width="100%" border="0">
	<tr>
		...
		<%
			...
			{
%>
		<td>This list shows <%=rowcount%> accounts worked in the month of <b><%=monthSelect.get(month)%> <%=year%></b></td>
		<%
			}
			else
			{
...
%>
		<td width="20%"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A HREF="<%= request.getContextPath() %>/admin/customer/showAccount.jsp?accountid=<%=accountid%>"><%= StringUtil.encodeHtmlTag(accountname)%></A></font></td>
