<%@ page language="java" contentType="text/html"
	pageEncoding="UTF-8"%>
<%@ page import="org.apache.log4j.*,pack.eminent.encryption.*,java.util.Calendar,java.text.*,com.eminent.util.*"%>
<%
	//Configuring log4j properties
	
	
	
	Logger logger = Logger.getLogger("Viewlead");
...
<table width="100%" align=center border="0" bgcolor="#F9F9F9">
	<tr>
		<td align=center>
		<FONT SIZE="4" COLOR="#33CC33">You have Successfully
		Moved a Lead to Opportunity : </FONT><FONT SIZE="4" COLOR="#0000FF"> <%=request.getParameter("lead")%> </FONT></td>
	</tr>
</table>
<%
	}
	...
	try
    {
		...
		if(connection != null)  
		{
			st = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
                        if(leadName != null){
                            if(!leadName.equalsIgnoreCase("")){
                                rs = st.executeQuery("SELECT LEADID,TITLE,FIRSTNAME, LASTNAME, COMPANY, STATE,PHONE,MOBILE,EMAIL,LEADSTATUS,RATING,DUEDATE,ASSIGNEDTO,CREATEDON,MODIFIEDON,LEAD_OWNER,lead_type from lead where roleid=2 and upper(firstname) like upper('%"+leadName+"%"+"') or upper(lastname) like upper('%"+leadName+"%"+"') or upper(company) like upper('%"+leadName+"%"+"') order by FIRSTNAME ASC ");
                                }else{
                                rs = st.executeQuery("SELECT LEADID,TITLE,FIRSTNAME, LASTNAME, COMPANY, STATE,PHONE,MOBILE,EMAIL,LEADSTATUS,RATING,DUEDATE,ASSIGNEDTO,CREATEDON,MODIFIEDON,LEAD_OWNER,lead_type from lead where roleid=2 order by upper(FIRSTNAME) ASC ");
                                }
                                }else if(rating!=null){
...
<table width="100%" align=center border="0" bgcolor="E8EEF7">
	<tr>
		<td align=center><FONT SIZE="4" COLOR="#33CC33">Lead
		has been Updated successfully!: </FONT> <FONT SIZE="4" COLOR="#0000FF"><%= request.getParameter("company")%></FONT>
		</td>
	</tr>
</table>
<%
...
<TABLE width="100%" border="0">
	<tr>
		<td>
                    <b><a href="javascript:void(0)">View Leads</a></b>&nbsp;&nbsp;&nbsp;&nbsp;
...
%>
	...
	<tr bgcolor="#E8EEF7" height="21">
		...
		<td width="18%" title="<%=  title%>"><input type="checkbox" name="approve" value="<%= leadid %>"><font
			...
			HREF="editLead.jsp?leadid=<%=leadid%>"><%= StringUtil.encodeHtmlTag(name)%></A></font>
		...
		<td width="18%" title="<%=  title%>"><input type="checkbox" name="approve" value="<%= leadid %>"><font
			face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A
			HREF="editLead.jsp?leadid=<%=leadid%>"><%=name.substring(0,25)+"..." %></A></font>
                 <%if (rs.getString("lead_type").equalsIgnoreCase("Influencer")) {%>
                    &nbsp;<img src="<%=request.getContextPath()%>/images/star.png"  alt="Influencer"  title="Influencer Lead"  style="cursor: pointer;height: 11px;"/>
                    <%} else if (rs.getString("lead_type").equalsIgnoreCase("Decision Maker")) {%>
                    &nbsp;<img src="<%=request.getContextPath()%>/images/hammer.png"  alt="Decision Maker"  title="Decision Maker Lead"  style="cursor: pointer;height: 11px;"/>
