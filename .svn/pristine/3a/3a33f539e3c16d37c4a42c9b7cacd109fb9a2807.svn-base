<%-- 
    Document   : newSAPExecutionPlan
    Created on : 7 Nov, 2011, 3:34:27 PM
    Author     : Tamilvelan
--%>
...
//Configuring log4j properties
    ...
    Logger logger = Logger.getLogger("SAP Execution Plan");
...
<html>
        ...
        <script type="text/javascript">
            ...
            function callManagers()
            {
                ...
                if (xmlhttp != null)
                {
                    ...
                    xmlhttp.open("GET", "managerDetails.jsp?product=" + prodId + "&rand=" + Math.random(1, 10), false);
            ...
            function callModules()
            {
                ...
                if (xmlhttpMod != null)
                {
                    ...
                    xmlhttpMod.open("GET", "moduleDetails.jsp?product=" + prodId + "&rand=" + Math.random(1, 10), false);
            ...
            function callMainProcess()
            {
                ...
                if (xmlhttp != null)
                {
                    ...
                    xmlhttp.open("GET", "mainProcessDetails.jsp?pId=" + prodId + "&rand=" + Math.random(1, 10), false);
                    xmlhttp.onreadystatechange = callbackMainProcess;
                    xmlhttp.send(null);
                }
            }
            function callbackMainProcess()
            {
                if (xmlhttp.readyState == 4)
                {
                    if (xmlhttp.status == 200)
                    {
                        ...
                        objLinkList = eval("document.getElementById('mp')");
                        objLinkList.length = 0;
                        ...
                        for (var i = 0; i < results.length - 1; i++)
                        {
                            ...
                            objLinkList.length++;
                            objLinkList[i].text = sp[1];
                            objLinkList[i].value = sp[0];
                            if (sp[1] == '--Select One--') {
            ...
            function callSubProcess()
            {
                ...
                if (xmlhttpMod != null)
                {
                    var prodId = document.getElementById('mp').value;

                    xmlhttpMod.open("GET", "subProcessDetails.jsp?mpId=" + prodId + "&rand=" + Math.random(1, 10), false);
                    xmlhttpMod.onreadystatechange = callbackSubProcess;
                    xmlhttpMod.send(null);
                }
            }
            function callbackSubProcess()
            {
                if (xmlhttpMod.readyState == 4)
                {
                    if (xmlhttpMod.status == 200)
                    {
                        ...
                        objLinkList = eval("document.getElementById('sp')");
                        objLinkList.length = 0;
            ...
            function callScenario()
            {
                ...
                if (xmlhttpMod != null)
                {
                    var spId = document.getElementById('sp').value;

                    xmlhttpMod.open("GET", "scenarioDetails.jsp?spId=" + spId + "&rand=" + Math.random(1, 10), false);
                    xmlhttpMod.onreadystatechange = callbackScenario;
                    xmlhttpMod.send(null);
                }
            }
            ...
            function callbackScenario()
            {
                if (xmlhttpMod.readyState == 4)
                {
                    if (xmlhttpMod.status == 200)
                    {
                        ...
                        objLinkList = eval("document.getElementById('sc')");
                        objLinkList.length = 0;
            ...
            function callVariant()
            {
                ...
                if (xmlhttpMod != null)
                {
                    var scId = document.getElementById('sc').value;

                    xmlhttpMod.open("GET", "variantDetails.jsp?scId=" + scId + "&rand=" + Math.random(1, 10), false);
                    xmlhttpMod.onreadystatechange = callbackVariant;
                    xmlhttpMod.send(null);
                }
            }
            ...
            function callbackVariant()
            {
                if (xmlhttpMod.readyState == 4)
                {
                    if (xmlhttpMod.status == 200)
                    {
                        ...
                        objLinkList = eval("document.getElementById('vt')");
                        objLinkList.length = 0;
 ...
 var plannameCheck;
    ...
    <body>
        <form name="executionpaln" onsubmit="return validate(this)" action="createSAPExecutionPlan.jsp" method="post">
            <table width="100%">
                <tr bgcolor="#C3D9FF">
            ...
            <%
                ...
                HashMap mProcess = ViewBPM.getClientMP(pId);
                LinkedHashMap mainProcess = GetProjectMembers.sortHashMapByKeys(mProcess, true);
                logger.info("mainProcess" + mainProcess);
            %>
            <table width="60%" bgColor=#E8EEF7 border="0" align="center">
                ...
                <tr>
                    <td><b>Build No</b></td>
                    <td><input type="text" id="buildno" name="buildno" /></td>
                ...
                <tr>
                    <td><b>Main Process</b></td>
                    <td colspan="3">
                        <select id="mp" name="mp" onchange="javascript:callSubProcess();">
                            <option value="">--Select One--</option>
                            <%Collection mpC = mainProcess.keySet();
                                Iterator itermp = mpC.iterator();
                                while (itermp.hasNext()) {
                                    int key = (Integer) itermp.next();
                                    String name = (String) mainProcess.get(key);
                            %>
                            <option value="<%=key%>"><%=name%></option>
                            <%}
                            %>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <select id="sp" name="sp" onchange="javascript:callScenario();">
                            <option value="">--Select One--</option>
                        </select>
                ...
                <tr>
                    <td><b>Scenario</b></td>
                    <td colspan="3">
                        <select id="sc" name="sc" onchange="javascript:callVariant();">
                            <option value="">--Select One--</option>
                        </select>
                    </td>
                </tr>
                <tr>
                    <td><b>Variant</b></td>
                    <td colspan="3">
                        <select id="vt" name="vt" onchange="javascript:callModules();">
                            <option value="">--Select One--</option>
                        </select>
                    </td>
                ...
                <tr>
                    <td align=right></td>
                    <td align=right><input type="hidden" name="project" id="project" value="<%=pId%>"><INPUT type=submit id="submit" value=Submit name=submit></td>
