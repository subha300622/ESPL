<%-- 
    Document   : CreatedIssues
    Created on : Oct 31, 2009, 8:17:00 PM
...
<%
	...
	Logger logger = Logger.getLogger("Created Issues");
...
<script>
    function monthSelection(){
        ...
        if(selectedValue>=startRange &&selectedValue<=current_date ){
            location = 'CreatedIssues.jsp?month='+month+'&year='+year;
        }else{
            alert("You can view your Created Issues from Aug 2008 to Current Month");
        }
    }
...
//        logger.info("Selected Month"+selectMonth);
 ...
 //       logger.info("MAX DAY of MOnth"+maxday);
        ...
        String issueDetails[][]     =   IssueDetails.displayIssues(Integer.parseInt(workeduserid), start, end, "Created");
...
<table width="100%">
	<tr height="10">
            <td align="left" width="100%">The <b>Time Taken</b> for each created issue in the month of <b><%=monthSelect.get(month)%> <%=year%></b> is listed below. Total created issues are <b> <%=rowcount%>.
		</b></td>
