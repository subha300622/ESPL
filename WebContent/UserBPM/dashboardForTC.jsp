<%-- 
    Document   : dashboardForTC
    Created on : 5 Nov, 2011, 9:56:00 PM
    Author     : Tamilvelan
--%>
...
<html>
    ...
    <body>
        <%
            ...
            HashMap tcase = ViewBPM.getTC(Integer.parseInt(vtId));
        ...
        <table>
            ...
            <tr>
                <td><span title="Client"><%=clientName%></span> >> <span title="Company Code"><%=compName%></span> >> <span title="Business Process"><%=nameofbp%></span> >> <span title="Main Process"><%=nameofmp%></span> >> <span title="Sub Process"><%=nameofsp%></span>  >> <span title="Scenario"><%=nameofsc%></span> >> <span title="Variant"><%=nameofvt%></span> >> <b>Test Case</b></td>
            </tr>
        </table>
        <%
            int sizeOftheTC = testcase.size();
            if (sizeOftheTC > 0) {
        %>
...
//                                    "paletteColors": "#0075c2,#1aaf5d,#f2c500,#f45b00,#8e0000",
            ...
            %>
                            {
                                "label": "<%=nameoftc%>",
                                "value": "<%= tsCount%>",
                                "link": '<%= request.getContextPath()%>/UserBPM/dashboardForTS.jsp?tcId=<%=key%>&pid=<%=pid%>&tcName=<%=nameoftc%>&vtName=<%=nameofvt%>&scName=<%=nameofsc%>&spName=<%=nameofsp%>&mpName=<%=nameofmp%>&bpName=<%=nameofbp%>&companyName=<%=compName%>&clientName=<%=clientName%>'
                                                    },
            <%}%>
                                                ]
                                            }
        ...
        <table cellpadding="0" cellspacing="0" width="100%" datapagesize="25">
            <tr>
                <td  align="center" width="100%"><font size="4" COLOR="#0000FF"><b>No Test Case Available</b></font></td>
            </tr>
        </table>
        <br>
        <br>
