...
function viewSubIssues(mainIssue) {

    $('#mainIssue'+mainIssue+' div').toggleClass('treeExpand');
    if ($('#mainIssue'+mainIssue).has("ul").length === 0) {
        if (xmlhttp !== null) {
            

            xmlhttp.open("GET", "/eTracker/admin/dashboard/viewSubIssue.jsp?mainIssue=" + mainIssue + "&rand=" + Math.random(1, 10), false);
            xmlhttp.onreadystatechange = function () {
                callbackSubIssues(mainIssue);
            };
            xmlhttp.send(null);
        }
    } else {
       $('#mainIssue'+mainIssue+' ul').toggle('slow');
    }

}

function viewSubIssue(mainIssue) {
...
function callbackSubIssues(mainIssue) {
    if (xmlhttp.readyState === 4)
    {
        if (xmlhttp.status === 200)
        {
            var comp = xmlhttp.responseText;
            $('#mainIssue'+mainIssue).append(comp);
