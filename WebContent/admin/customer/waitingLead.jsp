...
<%
	...
	session.setAttribute("forwardpage","waitingLead.jsp");
%>
...
<table cellpadding="0" cellspacing="0" width="100%" bgColor="#C3D9FF">
	<tr>
		<td align="left"><font size="4" COLOR="#0000FF"><b>Approve Lead
		</b> </font>
...
<% 
	...
	try
    {
		...
		if(connection != null)  
		{
			...
			rs = st.executeQuery("SELECT LEADID,TITLE,FIRSTNAME, LASTNAME, COMPANY, STATE,PHONE,MOBILE,EMAIL,LEADSTATUS,CREATEDON,MODIFIEDON,LEAD_OWNER from lead where roleid='"+x+"' and LEAD_OWNER != 'sales' order by UPPER(FIRSTNAME) ASC");
			rs.last();
...
<!--                <a	href="<%=request.getContextPath() %>/admin/customer/addnewlead.jsp">Add Lead</a>&nbsp;&nbsp;&nbsp;&nbsp; -->
                    ...
                    <b><a href="javascript:void(0)">Approve Leads</a></b>&nbsp;&nbsp;&nbsp;&nbsp;
                    <a  href="<%=request.getContextPath() %>/admin/customer/deniedLeads.jsp">Denied Leads</a>&nbsp;&nbsp;&nbsp;&nbsp;
                    <a  href="<%=request.getContextPath() %>/admin/customer/disabledLead.jsp">Disabled Leads</a>
                </td>
...
<%
                ...
                {
                %>
					<td width="18%" title="<%=  title%>"><input type="checkbox" name="approve" value="<%= leadid %>"><font	face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A HREF="editLead.jsp?leadid=<%=leadid%>"><%= StringUtil.encodeHtmlTag(name)%></A></font></td>
<%
                }
                else
                {
                %>
					<td width="18%" title="<%=  title%>"><input type="checkbox" name="approve" value="<%= leadid %>"><font	face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><A HREF="editLead.jsp?leadid=<%=leadid%>"><%=name.substring(0,25)+"..." %></A></font></td>
...
<%
                ...
                {
                %>
					<td width="13%"><font	face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%= StringUtil.encodeHtmlTag(company)%></font></td>
<%
                }
                else
                {
                %>
					<td width="13%"><font	face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%=company.substring(0,20)+"..." %></font></td>
...
<%if (email.length()<20)
  {
 %>
					<td width="18%"><font	face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%= StringUtil.encodeHtmlTag(email) %></font></td>
<%
  }else
  {
  %>
					<td width="18%"><font	face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#000000"><%= email.substring(0,20)+"..." %></font></td>
<%
  }
%>
...
%>
	<table align=right>
		<tr>
			<td>First</td>
			<td>Prev</td>
			<td><a href=waitingLead.jsp?manipulation=3>Next</a></td>
			<td><a href=waitingLead.jsp?manipulation=4>Last</a></td>
		</tr>
	</table>
	<%
			}
...
%>
	<table align=right>
		<tr>
			<td><a href=waitingLead.jsp?manipulation=1>First</a></td>
			<td><a href=waitingLead.jsp?manipulation=2>Prev</a></td>
			<td>Next</td>
			<td>Last</td>
		</tr>
	</table>
