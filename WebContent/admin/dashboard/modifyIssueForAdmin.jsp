...
<%@page import="com.eminent.util.UpdateIssue"%>
...
<%@page import="dashboard.Project"%>
<%@page import="com.eminent.util.UpdateValue"%>
...
<%@ page import="com.pack.MyAuthenticator,com.pack.ChangeFormat, com.eminent.validation.StatusValidation"%>
...
<%
    ...
    Logger logger = Logger.getLogger("Modify Issue for Admin");
...
<BODY onload="check()">
    ...
    <%
        ...
        Statement stmt1 = null, statementEmail1 = null, statusUpdate = null, stForType = null, stForProject = null, ischeckTypeIssue = null;
        ResultSet rs1 = null, resultSetEmail1 = null, rsForType = null, isRPtypeIssue = null;
            ...
            if (issueStatus.equalsIgnoreCase("QA-BTC") || addTestCase != null) {
                ...
                if (functionality != null) {
                    for (int i = 0; i < functionality.length; i++) {
                        ...
                        TqmUtil.createIssuePTC(pId, mId, functionality[i], description[i], expectedresult[i], userId, theissu);
        ...
        if (comments != "") {
            try {
                ...
                logger.error("Exception in issue commets:" + e);
            } finally {
                if (ps != null) {
                    ps.close();
                }
        ...
        try {
            ...
            if (usub != null && udes != null && uexpected_result != null) {
                logger.info("usub" + usub + ", udes" + udes + ", uexpected_result" + uexpected_result);
                ps = connection.prepareStatement("update issue set  severity=?, priority=?, assignedto=?, modifiedon=?, due_date = ?, rating = ?, feedback = ?, subject =?, description=?,estimated_time=?,sap_issue_type=?,EXPECTED_RESULT=?,ESCALATION=?,type=? " + extendedQuery + "  where issueid=?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                ...
                ps.setString(8, usub);
                ps.setString(9, udes);
                ps.setString(10, estimatedTime);
                ps.setString(11, sapIssueType);
                ...
                int i = 12;
                if (!fversion.equalsIgnoreCase(fix_version)) {
                    ps.setString(i, ProjectFinder.getProjectId(pName, fix_version));
                    ps.setString(i + 1, mid);
                    i = i + 2;
                } else if (newModuleId > 0) {
                    ps.setString(i, mid);
            ...
            logger.error("Exception in updating issues:" + e);
        } finally {
            if (ps != null) {
                ps.close();
            }
        ...
        try {
            if (issueStatus.equalsIgnoreCase("Closed")) {
                ...
                if (wrmid != 0) {
                    ...
                    ApmAdditionalClosed apmAdditionalClosed = new ApmAdditionalClosed(0l, theissu, apmWrmPlan);
            ...
            logger.error("Exception in ApmAdditionalClosed" + ex);
        }
        try {
        ...
        if (x > 0 || y > 0 || z > 0) {
            ...
            try {
                ...
                ArrayList<String> al = Project.getDetails(project + ":" + version);
                ArrayList<String> dateTime = CurrentDay.getDateTime();
                ...
                String Subject = "eTracker " + issueStatus + " Issue :  " + subject;
