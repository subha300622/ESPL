...
%>
<%Logger logger = Logger.getLogger("AssignedUserDetails");
    ...
    try {
        ...
        if (prod1.equals("'All Information'")) {
            ...
            rs1 = stmt1.executeQuery("select distinct firstname,lastname,assignedto from users,issue where to_char(userid) = assignedto order by firstname");
        } else  {/*edit by sowjanya*/
            
            stmt1 = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rs1 = stmt1.executeQuery("select distinct firstname,lastname,assignedto from users,issue,project where to_char(userid) = assignedto and project.pid=issue.pid and project.pname in(" + prod1 + ") order by firstname");
        } 
        ...
        while (rs1.next()) {
            assigned = rs1.getString("assignedto") + "--" + rs1.getString("firstname") + " " + rs1.getString("lastname");
