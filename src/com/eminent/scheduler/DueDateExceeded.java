 ...
 * @author Tamilvelan
 */
public class DueDateExceeded implements Job {

    static Logger logger = null;

    static {
        logger = Logger.getLogger("DueDateExceeded");
    }

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        System.out.println("DueDateExceeded");
        ...
        try {
            ...
            al = ScheduleHelper.getDueDateExceededUsers();
            ...
            for (int i = 0; i < userSize; i++) {
                ...
                int rowcolor = 0;
                dueDateExceededRS = statement.executeQuery("select i.issueid, pname as project, type, s.status, subject, description, priority, severity, createdby, createdon, assignedto, modifiedon, due_date,rating,feedback from issue i,issuestatus s,project p where i.issueid = s.issueid and  i.pid = p.pid and assignedto = '" + (String) al.get(i) + "' and s.status != 'Closed' and to_date(to_char(i.due_date,'DD-Mon-YYYY'),'DD-Mon-YYYY') < to_date(to_char(sysdate,'DD-Mon-yyyy'),'DD-Mon-YYYY')  order by due_date asc, modifiedon asc, type asc, priority asc, severity asc, createdon asc");

                dueDateExceededRS.last();
                int rowcount = dueDateExceededRS.getRow();
                dueDateExceededRS.beforeFirst();

                htmlContent = "<table><tr><td colspan=10><font	face=Verdana, Arial, Helvetica, sans-serif size=1 color=#000000>This list shows " + rowcount + " issue(s) exceeding the due date. Please do update the same at the earliest or coordinate with your Project Manager for extending the due date.</font> </td></tr>";
...
//                            msg.addRecipient(Message.RecipientType.BCC, new InternetAddress("ttamilvelan@eminentlabs.net"));
                ...
                msg.setSubject("Dear " + userName + ", Need Your Kind Attention on eTracker\u2122 Due Date Exceeded Issues in your MyAssignment", "UTF-8");
