 ...
 * @author ADAL
 */
public class ReassignIssues {
   
   static Logger logger=Logger.getLogger("Reassigning Issues");
   
    public static void reassignToPM(String[] args){
        ...
        for(int l=0;l<5;l++){
            ...
            for(Iterator r= al.iterator();r.hasNext();){
                ...
                String test     =   getUserIssues(projectid,userid,projectm);
            }
...
//				connection=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", "eminenttracker", "eminentlabs");
				...
				resultset=statement.executeQuery("select pid,pname,version,pmanager from project  where status='Work in progress' order by UPPER(PNAME) ASC");
                                resultset.last();
...
//				connection=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", "eminenttracker", "eminentlabs");
				statement=connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
				resultset=statement.executeQuery("select distinct assignedto from issue i ,issuestatus s where i.issueid=s.issueid and status!='Closed' and pid='"+pid+"' and assignedto!='"+pmanager+"' and due_date< (select sysdate from dual)  order by assignedto asc");
                                resultset.last();
                                int noOfRows    =   resultset.getRow();
                                resultset.beforeFirst();
...
public static String getUserIssues(String pid,String userid,String pmanager){
		...
		Statement statement=null;
		ResultSet resultset=null;
                String htmlContent    =   null,info=    null,   header= null;
                int rowcolor    =   0;
            ...
            //                        String assignedTo    =   (String)getUserDetail(assigned).get("username");
				...
				if(connection!=null) {
					...
					logger.error(ex.getMessage());
                        ...
                        String subject  = "eTracker™ Due Date Exceeded issues reassigned to your Project Manager";
                        SendMail.reassigningIssue(htmlContent, rowcolor, subject);
        return htmlContent;
    }
...
//            connection      =   MakeConnection.getConnection();
            ...
            issueStatement.setString(1,pmanager);
            issueStatement.setString(2, issueid);
