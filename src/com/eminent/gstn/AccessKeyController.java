...
public class AccessKeyController {
    ...
    public String getEInvoicingcalender(HttpServletRequest request) {
        ...
        try {
            ...
            connection = MakeConnection.getESPLASPConnection();
            statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            String query = "SELECT DATE(RequestTime),g.Category,COUNT(*)  FROM gstn_log g where Project ='" + project + "' AND DATE(RequestTime)  Between DATE('" + fromDate + "') and DATE('" + toDate + "') AND  AccessType='e-Invoice' \n" + "GROUP BY DATE(RequestTime),g.Category";
            resultSet = statement.executeQuery(query);
